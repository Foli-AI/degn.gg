# UPDATED: common/world/player/player.gd
# Changes: Added sendCoinUpdate() and sendFinishEvent() for backend integration
...
# Track last coin count to avoid duplicate sends
var last_coin_update: int = -1
...
func death(reason: String = "") -> void:
	...
	if coins > 0:
		...
		# Send coin update to backend after death
		send_coin_update()
	on_death()
...
# NEW: Send coin update to backend via ws-glue.js
func send_coin_update() -> void:
	if OS.has_feature("javascript"):
		# Only send if coin count changed
		if coins != last_coin_update:
			last_coin_update = coins
			JavaScript.eval("if (typeof window.sendCoinUpdate === 'function') { window.sendCoinUpdate(%d); }" % coins, true)
			Logger.print(self, "Sent coin update to backend: %d coins" % coins)


# NEW: Send finish event to backend via ws-glue.js
func send_finish_event() -> void:
	if OS.has_feature("javascript"):
		var player_id = str(self.name)
		# Send finish event to backend
		# The backend will track finish order and determine rankings
		JavaScript.eval("""
			if (typeof window.sendFinishEvent === 'function') {
				window.sendFinishEvent('%s');
			}
		""" % player_id, true)
		Logger.print(self, "Sent finish event to backend for player %s" % player_id)
