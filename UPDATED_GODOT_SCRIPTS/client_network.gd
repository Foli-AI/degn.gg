# UPDATED: client/client_network.gd
# Changes: Skip menus in HTML5 mode, use username from query params, go straight to world
...
# NEW: Get query parameter from URL (HTML5 only)
func _get_query_param(key: String) -> String:
...
func lost_connection(reason: String) -> void:
	...
	# In HTML5 mode, don't go back to title screen - just show error
	if OS.has_feature("javascript"):
		Globals.show_message(reason, "Connection Lost")
	else:
